...
openstack floating ip port forwarding create --internal-ip-address "$ip" \
--port "$(< IPP.txt)" --internal-protocol-port "$in" \
--external-protocol-port "$ex" --protocol tcp "$FLO" &&
openstack floating ip set "$FLO" --description "$(< description.txt)" &&
